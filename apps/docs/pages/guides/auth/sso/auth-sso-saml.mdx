...
This guide requires the use of the [Supabase CLI](/docs/guides/cli). Please make sure you're using version v1.46.0 or higher. You can use `supabase -v` to see the currently installed version.

SAML 2.0 support is off by default on Supabase projects. You can configure this on the [Auth Providers](https://app.supabase.com/project/_/auth/providers) page on your project.
...
Append `?download=true` to the Metadata URL to get a downloadable Metadata XML file. It's important to use the download form with identity providers that require a file!
...
Once a connection to an identity provider is established, you can remove it like so:

```bash
supabase sso remove <provider-id> --project-ref <your-project>
```

When successful, you will see the details of the removed identity provider. All user accounts from that identity provider will be immediately logged out. User information will remain in the system, but it will no longer be possible for any of those accounts to be accessed in the future, even if you add the connection again.
...
### How do I use SAML in local development?

Unfortunately using SAML in when developing on your machine is pretty difficult. This is a security limitation by identity providers which do not allow setting the SAML Assertion Consumer Service URL to non-HTTPS URLs, or they completely disallow the use of `localhost` in such settings.

You could get around these limitations by exposing your local Supabase stack to the internet and placing it behind a HTTPS server. Alternatively, you could use a locally hosted SAML 2.0 identity provider that will allow you to use insecure URLs.
