...
Unfortunately, it is difficult to use SAML when developing on your machine. This is a security limitation by identity providers which do not allow setting the SAML Assertion Consumer Service URL to non-HTTPS URLs, or they completely disallow the use of `localhost` in such settings.
